...
const cubies = [];
const EPS = 0.6; // prevents inaccuracies when selecting face cubies
let scrambleString = "";
...
function getFaceCubies(face) {
  switch (face) {
    case "R": return cubies.filter(c => c.position.x > EPS);
    case "L": return cubies.filter(c => c.position.x < -EPS);
    case "U": return cubies.filter(c => c.position.y > EPS);
    case "D": return cubies.filter(c => c.position.y < -EPS);
    case "F": return cubies.filter(c => c.position.z > EPS);
    case "B": return cubies.filter(c => c.position.z < -EPS);
    default: return [];
  }
}
...
export async function playSolution(moveSequence, cubeString) {
    ...
    return new Promise((resolve) => {
      ...
      function tick(now) {
        ...
        if (t < 1) {
          ...
          // set cubies back into scene after anim
          faceCubies.forEach((c) => {
  ...
  async function play() {
    for (let i = 0; i < moves.length; i++) {
      const mv = moves[i];
      const animMove = animateMove(mv);
      const highlightAnim = playMoveHighlight(moveTokens[i], animMove);
      await Promise.all([animMove, highlightAnim]);
